...
class JBTableViewController: UITableViewController  {
    ...
    override func viewDidLoad() {
        ...
        let location = CLLocationCoordinate2D(latitude: 27.224413888888886,longitude: -80.20233888888889)
        let span = MKCoordinateSpan(latitudeDelta: 0.075, longitudeDelta: 0.075)
        let region = MKCoordinateRegion(center: location, span: span)
        mapView.setRegion(region, animated: true)
        let annotation = MKPointAnnotation()
        annotation.coordinate = location
        annotation.title = "IRL-JB"
        annotation.subtitle = "Jenson Beach"
    ...
    func parseJSON() {
        ...
        let task = URLSession.shared.dataTask(with: url!) {(data, response, error) in
            ...
            if let array = json["JB"] as? [String] {
...
extension JBTableViewController {
