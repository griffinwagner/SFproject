...
class VBTableViewController: UITableViewController  {
    ...
    override func viewDidLoad() {
        ...
        let location = CLLocationCoordinate2D(latitude: 27.590141666666664,longitude: -80.35523888888888)
        let span = MKCoordinateSpan(latitudeDelta: 0.075, longitudeDelta: 0.075)
        let region = MKCoordinateRegion(center: location, span: span)
        mapView.setRegion(region, animated: true)
        let annotation = MKPointAnnotation()
        annotation.coordinate = location
        annotation.title = "IRL-VB"
        annotation.subtitle = "Vero Beach"
    ...
    func parseJSON() {
        ...
        let task = URLSession.shared.dataTask(with: url!) {(data, response, error) in
            ...
            if let array = json["VB"] as? [String] {
...
extension VBTableViewController {
